...
# major.minor.build (major or minor will invalidate the configuration)
CONFIG_VERSION = "V1.2.1"
HOME_ASSISTANT_PREFIX = "homeassistant"  # MQTT prefix used in autodiscovery

tempKeys = ["CellTemp1_4", "CellTemp5_8",
            "CellTemp9_12", "CellTemp13_16", "MOS_T", "ENV_T"]
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        log.info("MQTT connected OK Returned code=%s", rc)
        # subscribe to the commands
        try:
            topic = "{}{}/cmnd/#".format(
                argumentValues['mqttRoot'], argumentValues['rackName'])
            client.subscribe(topic)
            log.info("Subscribed to %s", topic)

            # publish that we are Online
            will_topic = "{}{}/tele/LWT".format(
                argumentValues['mqttRoot'], argumentValues['rackName'])
        ...
        mqttConnected = False
        log.error("MQTT Bad connection Returned code=%s", rc)
...
def on_disconnect(client, userdata, rc):
    ...
    if rc != mqttclient.MQTT_ERR_SUCCESS:
        log.info("on_disconnect: Disconnected. ReasonCode={}".format(
            mqttclient.error_string(rc)))
    ...
    if msg == "{\"STOP\"}":
        ...
        if "publishRate" in theMessage:
            ...
            if newRate < MIN_PUBLISH_RATE:
                log.error("Received publishRate of {} which is below minimum of {}".format(
                    newRate, MIN_PUBLISH_RATE))
            elif newRate > MAX_PUBLISH_RATE:
                log.error("Received publishRate of {} which is above maximum of {}".format(
                    newRate, MAX_PUBLISH_RATE))
            else:
                argumentValues['publishRate'] = newRate
                currentPollRate = newRate
                log.info(
                    "publishRate message received, setting rate to {}".format(newRate))
        else:
...
def mqttPublish(data, subtopic, retain):
    ...
    topic = "{}{}/stat/{}".format(argumentValues['mqttRoot'],
                                  argumentValues['rackName'], subtopic)
...
def PublishDiscoverySub(component, entity, jsonElement, device_class, unit_of_meas, icon=0):
    ...
    if (icon):
        doc["icon"] = icon
    doc["state_topic"] = "{}{}/stat/readings/Pack{}".format(
        argumentValues['mqttRoot'], argumentValues['rackName'], current_pack_number)
    ...
    doc["availability_topic"] = "{}{}/tele/LWT".format(
        argumentValues['mqttRoot'], argumentValues['rackName'])
    doc["pl_avail"] = "Online"
    ...
    device["identifiers"] = "Pack{}_{}".format(
        current_pack_number, pack_barcodes[current_pack_index])
    doc["device"] = device
    mqttClient.publish("{}/{}/{}/config".format(HOME_ASSISTANT_PREFIX, component, object_id),
                       json.dumps(doc, sort_keys=False, separators=(',', ':')), qos=0, retain=False)
...
def PublishTempsDiscovery(numberOfTemps):
    for x in range(numberOfTemps):
        ...
        if (x < len(tempKeys)):
            tempKey = tempKeys[x]
        PublishDiscoverySub("sensor",  tempKey, "Temps.{}.Reading".format(
            tempKey), "temperature", "°C")


def PublishCellsDiscovery(numberOfCells):
    for x in range(numberOfCells):
        PublishDiscoverySub("sensor",  "Cell_{}".format(
            x+1), "Cells.Cell_{}.Reading".format(x+1), "voltage", "V", "mdi:lightning-bolt")


def publishDiscovery(pylonData):
    global current_pack_index

    PublishDiscoverySub("sensor", "PackVoltage",
                        "PackVoltage.Reading", "voltage", "V", "mdi:lightning-bolt")
    PublishDiscoverySub("sensor", "PackCurrent",
                        "PackCurrent.Reading", "current", "A", "mdi:current-dc")
    PublishDiscoverySub("sensor", "SOC", "SOC", "battery", "%", icon=0)
    PublishDiscoverySub("sensor", "RemainingCapacity",
                        "RemainingCapacity", "current", "Ah", "mdi:ev-station")
...
def periodic(polling_stop):
    ...
    if not polling_stop.is_set():
            ...
            if mqttConnected:
                if pylontech is None:
                    if argumentValues['sok']:
                        pylontech = PylonTechSOK(
                            argumentValues['pylonPort'], int(argumentValues['baud_rate']))
                    else:
                        pylontech = Pylontech(argumentValues['pylonPort'], int(
                            argumentValues['baud_rate']))
                ...
                if number_of_packs == 0:
                    ...
                    if not info_published[current_pack_index]:
                        ...
                        if vi:
                            ...
                            if bc:
                                mqttPublish(encodePylon_info(
                                    vi, bc), "info/Pack{}".format(current_pack_number), True)
                                info_published[current_pack_index] = True
                                pack_barcodes[current_pack_index] = bc.Barcode
                    pylonData = pylontech.get_values_single(
                        current_pack_number)
                    ...
                    if pylonData:  # got data
                        mqttPublish(encodePylon_readings(
                            pylonData, ai), "readings/Pack{}".format(current_pack_number), False)
        ...
        threading.Timer(timeUntilNextInterval, periodic,
                        [polling_stop]).start()
...
def run(argv):
    ...
    clientId = argumentValues['mqttUser'] + \
        "_mqttclient_" + str(randint(100, 999))
    log.info("Connecting with clientId=" + clientId)
    mqttClient = mqttclient.Client(clientId)
    mqttClient.username_pw_set(
        argumentValues['mqttUser'], password=argumentValues['mqttPassword'])
    ...
    try:
        log.info("Connecting to MQTT {}:{}".format(
            argumentValues['mqttHost'], argumentValues['mqttPort']))
        mqttClient.connect(host=argumentValues['mqttHost'], port=int(
            argumentValues['mqttPort']))
    except Exception as e:
        ...
        try:
            ...
            if not mqttConnected:
                if (mqttErrorCount > MQTT_MAX_ERROR_COUNT):
                    log.error(
                        "MQTT Error count exceeded, disconnected, exiting...")
                    doStop = True
