...
class Pylontech:
    ...
    get_alarm_fmt = construct.Struct(
        ...
        "CellState" /
        construct.Array(construct.this.NumberOfCells, construct.Int8ub),
        "NumberOfTemperatures" / construct.Int8ub,
        "CellsTemperatureStates" /
        construct.Array(construct.this.NumberOfTemperatures, construct.Int8ub),
    ...
    get_analog_fmt = construct.Struct(
        ...
        "NumberOfCells" / construct.Int8ub,
        "CellVoltages" /
        construct.Array(construct.this.NumberOfCells,
                        ToVolt(construct.Int16sb)),
        "NumberOfTemperatures" / construct.Int8ub,
        "GroupedCellsTemperatures" /
        construct.Array(construct.this.NumberOfTemperatures,
                        ToCelsius(construct.Int16sb)),
        "Current" / ToAmp(construct.Int16sb),
        "Voltage" / ToVolt(construct.Int16ub),
        "Power" /
        Round1(construct.Computed(construct.this.Current * construct.this.Voltage)),
        "_RemainingCapacity" / construct.Int16ub,
        "RemainingCapacity" /
        DivideBy100(construct.Computed(construct.this._RemainingCapacity)),
        "_UserDefinedItems" / construct.Int8ub,
        "TotalCapacity" / DivideBy100(construct.Int16ub),
        "CycleNumber" / construct.Int16ub,
        "StateOfCharge" /
        Round1(construct.Computed(
            construct.this._RemainingCapacity / construct.this.TotalCapacity)),
    )

    def __init__(self, serial_port='/dev/ttyUSB0', baudrate=9600):
        self.s = serial.Serial(serial_port, baudrate, bytesize=8,
    ...
    def _encode_cmd(self, address: int, cid2: int, info: bytes = b''):
        ...
        frame = "{:02X}{:02X}{:02X}{:02X}{:04X}".format(
            0x20, address, cid1, cid2, info_length).encode()
        frame += info
        frame_chksum = Pylontech.get_frame_checksum(frame)
        whole_frame = (
            b"~" + frame + "{:04X}".format(frame_chksum).encode() + b"\r")
        return whole_frame
...
class PylonTechSOK(Pylontech):
    ...
    def _encode_cmd(self, address: int, cid2: int, info: bytes = b''):
        ...
        info_length = Pylontech.get_info_length(info)
        frame = "{:02X}{:02X}{:02X}{:02X}{:04X}".format(
            0x25, address, cid1, cid2, info_length).encode()
        frame += info
        frame_chksum = Pylontech.get_frame_checksum(frame)
        whole_frame = (
            b"~" + frame + "{:04X}".format(frame_chksum).encode() + b"\r")
